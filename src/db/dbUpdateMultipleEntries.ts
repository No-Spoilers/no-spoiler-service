...
export default async function dbUpdateMultipleEntries(entryList: EntryList, userId: string): Promise<unknown | null> {
  try {
    const items: Partial<UpdateItemCommandInput>[] = entryList.mentions.map(entry => {
